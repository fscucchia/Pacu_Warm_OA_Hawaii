...
under_label_data <- data.frame(
  x = -3000, # Adjust x position as needed
...
```{r}
# Plot
plot1 <- ggplot(ress_filt, aes(delta.rank.x, delta.rank.y, label = name.y)) +
...
# Set 1 
### BP 
set1_bp <- merge(bp_hvc_filt, bp_mvc_filt, by = "term") # The x is High v Control and the y is Mid v Control  
set1_bp_filt <- set1_bp %>%
  dplyr::filter(p.adj.x < 0.01) %>%
  dplyr::filter(p.adj.y < 0.01) %>%
  dplyr::filter(!grepl("obsolete", name.x))

# Calculate top positive delta ranks
top_ranks <- set1_bp_filt %>%
  filter(delta.rank.x > 0 & delta.rank.y > 0) %>%
  arrange(desc(delta.rank.x + delta.rank.y)) %>%
  head(7) %>%
  pull(name.y)  # Get only the names for the label

# Create a string for the label
#top_ranks_label <- paste(top_ranks, collapse = "\n")
top_ranks_label <- c("DNA amplification", "DNA replication initiation", "telomere maintenance of semi-conservative replication", "regulation of interferon-alpha production")
top_ranks_label_test <- paste(top_ranks_label, collapse = "\n")

# Create a data frame for the label position
label_data <- data.frame(
  x = 100, # Adjust x position as needed
  y = 1800, # Adjust y position as needed
  label = top_ranks_label_test
)

# Calculate top negative delta ranks
neg_ranks <- set1_bp_filt %>%
  filter(delta.rank.x < 0 & delta.rank.y < 0) %>%
  arrange(desc(delta.rank.x + delta.rank.y)) %>%
  head(7) %>%
  pull(name.y)  # Get only the names for the label
...
# Create a string for the label
#neg_ranks_label <- paste(neg_ranks, collapse = "\n")
neg_ranks_label <- c("mitochondrial ATP synthesis coupled electron transport", "oxidative phosphorylation", "cellular respiration", "electron transport chain", "nucleoside triphosphate metabolic process")
neg_ranks_label_test <- paste(neg_ranks_label, collapse = "\n")

# Create a data frame for the label position
neg_label_data <- data.frame(
  x = -2400, # Adjust x position as needed
  y = -1500, # Adjust y position as needed
  label = neg_ranks_label_test
)


# Plot
plot1 <- ggplot(set1_bp_filt, aes(delta.rank.x, delta.rank.y, label = name.y)) +
  geom_point(size = 2, show.legend = FALSE) +
  scale_size("size") +
  scale_x_continuous(limits = c(-2500, 2000), breaks = seq(-2000, 2000, 1000)) +
  scale_y_continuous(limits = c(-2000, 2400), breaks = seq(-2000, 2000, 1000)) +
...
ggsave("../output/GO_MWU/HighvControl_MidvControl_BP_DeltaRank_boxlabel.pdf", plot = plot1, width = 20, height = 15, units = "cm")
ggsave("../output/GO_MWU/HighvControl_MidvControl_BP_DeltaRank_boxlabel.png", plot = plot1, width = 20, height = 15, units = "cm")
```
...
Pull most significant delta ranks
```{r}
head(ress_filt)
tail(ress_filt)
```

Use info from head and tail to make text boxes--also maybe arrange in a different way?  


Plot before calculating positive and neg delta ranks 
```{r}
bp_plot <- ggplot(ress_filt, aes(delta.rank.x, delta.rank.y, label = name.y)) +
  geom_point(size = 2.5, show.legend = FALSE) +
  geom_text_repel(
    ...
    box.padding = 0.7,
    direction = "both",
    max.overlaps = 20, # Allow more overlaps but control with point.size
    size = 3, # Reduce text size
    force = 2 # Increase repulsion force
  ) +
  scale_size("size") +
...
# Create a data frame for the label position
label_data <- data.frame(
  x = 1750, # Adjust x position as needed
  y = 2000, # Adjust y position as needed
  label = top_ranks_label
)
```

Negative delta ranks 
```{r}
# Calculate top negative delta ranks
neg_ranks <- ress_filt %>%
  filter(delta.rank.x < 0 & delta.rank.y < 0) %>%
...
# Create a data frame for the label position
neg_label_data <- data.frame(
  x = -2500, # Adjust x position as needed
  y = -500, # Adjust y position as needed
  label = neg_ranks_label
)
```
